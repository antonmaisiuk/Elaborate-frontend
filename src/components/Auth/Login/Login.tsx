...
const Login = () => {
	...
	const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			setErrorMsg(
				`Unexpected error. Please try again with correct credentials.`
			);
			setSpinnerActive(false);
		}
	};
	...
	const handle2FA = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		const response = await fetch(
			`https://localhost:7247/api/Authentication/login-2FA?code=${formData.code}&email=${formData.email}`,
			{
				method: 'Post',
				headers: { 'Content-Type': 'application/json' },
			}
	...
	return (
		<AuthContainer>
			<AuthWrapper>
					...
					{is2FA ? (
						...
						<>
							...
							<StyledFormGroup controlId='password'>
								<StyledFormLabel>
									Password
									<StyledFogotPassword onClick={() => navigate('/forgot')}>
										Forgot Password?
									</StyledFogotPassword>
								</StyledFormLabel>
				...
				<StyledOption>or sign up with</StyledOption>
				<StyledGoogleButton onSuccess={responseMessage} />
				<StyledOption>
					{is2FA ? (
						<StyledLink onClick={() => setIs2FA(false)}>
							Back to login
						</StyledLink>
					) : (
